  ...
  admin: {
    ...
    users: {
      ...
      platformQuota: {
        ...
        updateFailed: '保存失败',
        loadFailed: '加载失败',
        hint: '留空 = 不限制该窗口；填 0 = 禁用该平台该窗口。',
        windowDaily: '日',
        windowWeekly: '周',
        windowMonthly: '月',
        cellNotConfigured: '未配置',
        cellColumnTooltip: '仅展示已设置限额的平台',
    ...
    accounts: {
      ...
      alertConfig: {
        menuTitle: '账号告警配置',
        title: '账号告警配置',
        description: '当账号定时检测失败、触发限流、进入错误状态、限制恢复，或全局/分组可调度账号数量不足时，通过邮件提醒管理员。',
        enabled: '启用账号告警',
        enabledHint: '关闭后不会发送账号不可用、限流或可用数量不足提醒。',
        recipientsTitle: '收件人',
        useOpsRecipients: '使用运维监控收件人',
        useOpsRecipientsHint: '优先沿用「运维监控」里的邮件通知配置。',
        extraRecipients: '额外收件人',
        extraRecipientsPlaceholder: '每行一个邮箱，也支持用逗号分隔',
        extraRecipientsHint: '额外收件人会和运维监控收件人合并去重。',
        cooldownTitle: '提醒频率',
        cooldownMinutes: '冷却时间（分钟）',
        checkIntervalMinutes: '数量检查间隔（分钟）',
        cooldownHint: '定时检测失败仍按冷却时间去重；账号限制只在每次进入限制时提醒一次，恢复后会发送恢复提醒并解除去重。',
        rulesTitle: '告警规则',
        scheduledTestFailure: '定时检测失败',
        scheduledTestFailureHint: '账号定时测试返回失败时提醒。',
        rateLimitFailure: '账号限流',
        rateLimitFailureHint: '账号进入限流、过载或临时不可调度窗口时提醒；恢复后也会提醒。',
        errorFailure: '账号错误/不可用',
        errorFailureHint: '账号进入错误状态或非限流失败时提醒。',
        minAvailableAccounts: '可用账号数量不足',
        minAvailableAccountsHint: '后台定时检查全局和每个分组的可调度账号数量，低于阈值时提醒。',
        minAvailableThreshold: '最低可调度账号数',
        loadFailed: '账号告警配置加载失败',
        saveSuccess: '账号告警配置已保存',
        saveFailed: '账号告警配置保存失败'
      },
      dataExportConfirmMessage: '导出的数据包含账号与代理的敏感信息，请妥善保存。',
      dataExportConfirm: '确认导出',
      dataExported: '数据导出成功',
      dataExportFailed: '数据导出失败',
      refreshTokenSuccess: '账号令牌刷新成功',
      refreshTokenFailed: '账号令牌刷新失败',
      refreshTokenNeedsReauth: '该账号的 Refresh Token 已失效或已被使用过，无法继续刷新。请使用“重新授权”获取新的令牌。',
      ...
      bulkDataImportTitle: '批量导入',
      bulkDataImportHint: '选择一个或多个账号导出 JSON 文件，系统会按普通导入逻辑逐个导入。',
      bulkDataImportWarning: '导入会创建文件内的账号和代理。只支持普通“导出”生成的 JSON 文件，不支持文本粘贴。',
      bulkDataImportFiles: 'JSON 文件',
      bulkDataImportButton: '开始导入',
      bulkDataImporting: '导入中...',
      bulkDataImportSelectFiles: '请选择 JSON 文件',
      bulkDataImportSelectedFiles: '已选择 {count} 个文件',
      bulkDataImportEmptyFile: '文件内容为空',
      bulkDataImportParseFailed: 'JSON 解析失败',
      bulkDataImportFailed: '批量导入失败',
      bulkDataImportResult: '导入结果',
      bulkDataImportResultSummary: '文件 {files_total} 个，成功 {files_imported}，失败 {files_failed}；账号新增 {account_created}，账号失败 {account_failed}，代理新增 {proxy_created}，代理复用 {proxy_reused}，代理失败 {proxy_failed}',
      bulkDataImportErrors: '失败详情',
      bulkDataImportSuccess: '导入完成：文件 {files_imported} 个，账号新增 {account_created}，代理新增 {proxy_created}',
      bulkDataImportCompletedWithErrors: '导入完成但有错误：文件失败 {files_failed}，账号失败 {account_failed}，代理失败 {proxy_failed}',
    ...
    settings: {
      ...
      site: {
        ...
        ccsDefaultModel: 'CCS 默认模型',
        ccsDefaultModelPlaceholder: '例如 gpt-5.4',
        ccsDefaultModelHint: '导入 OpenAI/Codex 密钥到 CCS 时使用，留空会恢复默认值。',
